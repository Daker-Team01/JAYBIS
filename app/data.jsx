...
function analyzeCashflow(transactions = DUMMY_TRANSACTIONS) {
  ...
  const savingsPotential = Math.max(0, Math.round((fixedItems.filter(t => ['구독', 'OTT'].includes(t.category)).reduce((sum, t) => sum + Math.abs(t.amount), 0) + variable * 0.12) / 100) * 100);
...
// 에이전트가 질문별로 쓰는 도구 호출 시퀀스
const CHAT_TOOLSEQ = [
  { tool: '거래내역 조회', detail: '최근 30일 · 카드+계좌', ms: 900 },
  { tool: 'MCC 카테고리 분류', detail: '거래 유형 자동 분류', ms: 1100 },
  { tool: '고정비 자동 식별', detail: '구독 · 정기결제 탐지', ms: 800 },
  { tool: '진단 코멘트 생성', detail: '쉬운 설명으로 요약', ms: 700 },
];
