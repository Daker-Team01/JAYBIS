...
function normalizeProducts(products = []) {
  return Array.isArray(products)
    ? products.map((product, index) => ({
        ...
        minMonthly: numberOrZero(product.minMonthly ?? product.min_monthly),
        maxMonthly: numberOrZero(product.maxMonthly ?? product.max_monthly),
        minTermMonths: numberOrZero(product.minTermMonths ?? product.min_term_months),
        maxTermMonths: numberOrZero(product.maxTermMonths ?? product.max_term_months),
...
function normalizeSupabaseFinancialProduct(row = {}, index = 0) {
  ...
  return normalizeProducts([{
    ...
    simulation,
    coaching,
    minMonthly: row.min_monthly ?? row.minMonthly ?? limits.minMonthly ?? limits.min_monthly,
    maxMonthly: row.max_monthly ?? row.maxMonthly ?? limits.maxMonthly ?? limits.max_monthly,
    minTermMonths: row.min_term_months ?? row.minTermMonths ?? limits.minTermMonths ?? limits.min_term_months,
    maxTermMonths: row.max_term_months ?? row.maxTermMonths ?? limits.maxTermMonths ?? limits.max_term_months,
