...
const {
  useState, Logo, Icon, SectionLabel, stagger,
  USER, ASSETS, AI_DIAGNOSIS, won, manwon, pct,
} = window;
          ...
          >
            <span
              style={{
                ...
                alignItems:'center',
                justifyContent:'center',
                boxShadow:'0 4px 12px rgba(0,71,187,.32)'
              }}
            >
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:11 }}>
            {FEATURES.map((f, i) => (
              <button
                ...
                onClick={() => nav(f.go)}
                className="card"
                style={{ textAlign:'left', padding:'16px 15px' }}
              >
                <span
                  className="ic-chip"
                  style={{
        ...
        <div style={{ marginTop:24, ...stagger(3) }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            ...
            <ActionCard
              ...
              icon="chat"
              title="구독 3개가 겹쳐요"
              body="하나만 남기면 월 18,500원 절약"
              onClick={() => nav('chat', '구독 정리하고 싶어')}
            />
          </div>
        </div>
      </div>
...
function MiniGauge({ value, label, txt, warn }) {
  ...
  return (
    <div style={{ textAlign:'center' }}>
      ...
      <div style={{ fontSize:11, color:'rgba(255,255,255,.78)', fontWeight:600, marginTop:5 }}>
        {label}
      </div>
    </div>
  );
